  ...
  return (
    <nav className="absolute z-[100] top-0 left-0 w-full py-4">
      <div className="container mx-auto flex justify-between items-center px-6 md:px-24">
        {/* Logo */}
        <div>
          <img src={isDark ? `images/Logo.png` : 'images/LightLogo.png'} alt="" className="w-24 md:w-32" />
        </div>
        ...
        <ul className="hidden md:flex space-x-6 items-center">
          {['about', 'skills', 'projects', 'contact'].map((item) => (
            <li key={item}>
              <a
                href={`#${item}`}
                className={`relative ${isDark ? 'text-gray-200 hover:text-white' : 'text-gray-600 hover:text-orange-600'} after:content-[''] after:absolute after:left-0 after:bottom-[-4px] after:w-0 after:h-[2px] after:rounded-2xl ${isDark ? 'after:bg-gray-300' : 'after:bg-orange-500'} after:transition-all after:duration-300 hover:after:w-full capitalize`}
              >
        ...
        <div className="hidden md:flex items-center gap-6">
          ...
          >
            <div
              className={`absolute top-1 ${isDark ? 'left-1' : 'left-[38px]'} w-7 h-7 bg-white rounded-full shadow-lg transition-all duration-300 flex items-center justify-center`}
            >
              {isDark ? (
        ...
        <div className="flex md:hidden items-center gap-3">
          <button
            ...
            aria-label="Toggle theme"
          >
            <div className={`absolute top-1 ${isDark ? 'left-1' : 'left-[30px]'} w-6 h-6 bg-white rounded-full shadow-lg transition-all duration-300 flex items-center justify-center`}>
              {isDark ? (
                <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 text-gray-700" fill="currentColor" viewBox="0 0 24 24">
      ...
      {menuOpen && (
        <div className={`md:hidden px-6 py-4 ${isDark ? 'bg-[#00000f]/95' : 'bg-[#efeae3]/95'} backdrop-blur-sm flex flex-col gap-4`}>
          {['about', 'skills', 'projects', 'contact'].map((item) => (
            <a
              key={item}
              href={`#${item}`}
              onClick={() => setMenuOpen(false)}
              className={`text-lg font-medium ${isDark ? 'text-gray-200' : 'text-gray-700'} capitalize`}
            >
              {item === 'about' ? 'About Me' : item.charAt(0).toUpperCase() + item.slice(1)}
            </a>
          ))}
